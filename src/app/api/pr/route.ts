...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!userHasCredits) {
      return NextResponse.json(
        { error: 'No press release credits available. Please purchase credits to create a release.' },
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    await db.delete(releaseCategories).where(eq(releaseCategories.releaseId, existingRelease.id))
    const allCategories: number[] = []
    if (topcat) {
      allCategories.push(parseInt(topcat))
    }
    ...
    await db.delete(releaseRegions).where(eq(releaseRegions.releaseId, existingRelease.id))
    if (selectedRegions && Array.isArray(selectedRegions) && selectedRegions.length > 0) {
      await db.insert(releaseRegions).values(
        selectedRegions.map((regionId: number) => ({
          releaseId: existingRelease.id,
    ...
    return NextResponse.json({ uuid: existingRelease.uuid, id: existingRelease.id })
  } catch (error) {
