...
export default async function CrmContactsPage({
  ...
  searchParams: Promise<{ page?: string; perPage?: string; q?: string; status?: string; type?: string }>
}) {
  const { uuid } = await params
  const { page: pageStr, perPage: perPageStr, q, status: statusParam, type: typeParam } = await searchParams
  ...
  const contactType = ['media', 'advocate'].includes(typeParam || '') ? typeParam! : ''
  ...
  const data = await getContacts(co.id, page, perPage, query, status, contactType)
