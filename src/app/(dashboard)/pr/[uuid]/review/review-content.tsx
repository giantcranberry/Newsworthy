...
export function ReviewContent({
  ...
  const [aiCopyImprovements, setAiCopyImprovements] = useState<CopyImprovement[]>([])
  ...
  const loadAIAnalysis = async (forceRefresh = false) => {
    ...
    try {
      ...
      if (!forceRefresh) {
        const cacheResponse = await fetch(`/api/pr/${releaseUuid}/ai-suggestions`)
  ...
  const bodyWordCount = useMemo(() => {
    ...
    const plainText = release.body.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim()
  ...
  const validationItems = useMemo<ValidationItem[]>(() => {
    ...
    if (!release.abstract) {
      items.push({
        ...
        description: 'Add a brief summary to help readers quickly understand your news.',
        editPath: `/pr/${releaseUuid}`,
        icon: FileText,
      })
    }
    ...
    if (stats.categoryCount === 0) {
      items.push({
        ...
        description: 'Select at least one category to help journalists find your news.',
        editPath: `/pr/${releaseUuid}`,
        icon: Tag,
      })
    }
    ...
    if (!banner) {
      items.push({
        ...
        description: 'A banner image increases social sharing engagement by up to 200%.',
    ...
    if (!release.pullquote) {
      items.push({
        ...
        description: 'A compelling quote makes your release more quotable by journalists.',
        editPath: `/pr/${releaseUuid}`,
        icon: Quote,
      })
    }
    ...
    if (!release.location) {
      items.push({
        ...
        description: 'Adding a location provides geographical context for your news.',
        editPath: `/pr/${releaseUuid}`,
        icon: MapPin,
      })
    }
    ...
    if (!release.releaseAt) {
      items.push({
        ...
        description: 'Schedule a specific release date or your release will go out immediately upon approval.',
        editPath: `/pr/${releaseUuid}`,
        icon: Calendar,
      })
    }
    ...
    if (stats.listCount > 0 && !stats.shareWithList) {
      ...
      items.push({
        ...
        description: 'Consider building a subscriber list to amplify your press releases.',
        editPath: `/pr/${releaseUuid}/share`,
        icon: Share2,
      })
    }
    ...
    if (!stats.distribution) {
      items.push({
        ...
        description: 'Expand your reach with premium news distribution upgrades.',
    ...
    if (abstractLength > 0 && abstractLength < 100) {
      items.push({
        ...
        description: 'Your abstract is quite short. Consider expanding it to 150-300 characters for better SEO.',
        editPath: `/pr/${releaseUuid}`,
        icon: FileText,
      })
    }
    ...
    return items
  }, [release, company, contact, banner, images, stats, releaseUuid, bodyWordCount, abstractLength])
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  item.completed ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'
                )}
              >
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center p-3 bg-gray-50 rounded-lg">
              <p className="text-2xl font-bold text-gray-900">{bodyWordCount}</p>
            ...
            <div className="text-center p-3 bg-gray-50 rounded-lg">
              <p className="text-2xl font-bold text-gray-900">{stats.regionCount}</p>
        ...
        <CardContent className="space-y-6">
          {isLoadingAI && (
            <div className="flex flex-col items-center justify-center py-8">
              ...
              <p className="text-sm text-gray-400 mt-1">Generating SEO and content optimization suggestions</p>
            </div>
          )}
            ...
            <>
              ...
              {aiSuggestions.length > 0 && (
                <div className="space-y-3">
                  ...
                  <div className="p-3 bg-gray-50 rounded-lg">
                    ...
                    <p className="font-medium text-gray-900">{release.title || 'Untitled'}</p>
                  </div>
                  <div className="space-y-3">
                ...
                <div className="space-y-3">
                  ...
                  <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                    ...
                    <p className="text-xs text-green-600 mt-2">
                      {aiAbstract.length}/350 characters - Add this summary to your press release
                    </p>
                  </div>
                </div>
              )}
              ...
              {aiPullquote && !release.pullquote && (
                <div className="space-y-3">
                  ...
                  <div className="p-4 bg-purple-50 border border-purple-200 rounded-lg">
                    <p className="text-purple-900 italic">&ldquo;{aiPullquote}&rdquo;</p>
                    <p className="text-xs text-purple-600 mt-2">
                      Consider adding this quote to make your release more quotable
                    </p>
                  </div>
                </div>
              )}
              ...
              {aiBrandableChunks.length > 0 && (
                <div className="space-y-3">
                  ...
                  <p className="text-sm text-gray-600">
                    Content segments that AI systems and search engines will likely extract for indexing.
                  </p>
                  <div className="space-y-4">
                      ...
                      >
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className={cn(
                            ...
                            chunk.brandability === 'High' ? 'bg-green-100 text-green-700' :
                            chunk.brandability === 'Medium' ? 'bg-amber-100 text-amber-700' :
                        ...
                        <div className="text-sm">
                          <p className="text-gray-600"><span className="font-medium">Issue:</span> {chunk.currentIssue}</p>
                          <p className="text-blue-600 mt-1"><span className="font-medium">Recommendation:</span> {chunk.recommendation}</p>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {hasLoadedAI && aiSuggestions.length === 0 && aiCopyImprovements.length === 0 && aiBrandableChunks.length === 0 && (
                <p className="text-center text-gray-500 py-4">No optimization suggestions available.</p>
              )}
          ...
          {!release.title || !release.body ? (
            <p className="text-center text-gray-500 py-4">
              Add a headline and body content to receive AI optimization suggestions.
            </p>
          ) : null}
        </CardContent>
      </Card>
      ...
      {!requiredComplete && (
        <div className="flex items-start gap-2 p-4 bg-red-50 border border-red-200 rounded-lg">
          ...
          <div className="text-sm text-red-800">
            <strong>Cannot proceed.</strong> Please complete all required items above before finalizing your press release.
          </div>
        </div>
      )}
